...
class WasmTaskTestCase(TestCase):
    def setUp(self):
        task_def = WasmTaskBuilder.build_full_definition(
            WasmTaskTypeInfo(), TEST_TASK_DEFINITION_DICT,
        )
    ...
    def test_get_next_subtask_extra_data(self):
        ...
        self.assertEqual(subt_extra_data, {
            ...
            'js_name': 'test.js',
            'wasm_name': 'test.wasm',
            'script_filepath': WasmTask.JOB_SCRIPT_PATH,
            'exec_args': ['arg1', 'arg2'],
            'input_dir_name': 'dir',
            'output_file_paths': ['file1', 'file2'],
        })
    ...
    def test_accept_results(self):
        ...
        res_f = [
            os.path.join(result_dir.name, 'file1'),
            os.path.join(result_dir.name, 'file2'),
        ]
        res_f_contents = [
            bytes([0, 1, 2, 3, 4, 5]),
        ...
        exp_out_f = [
            os.path.join(output_dir.name, 'test_subtask_name', 'file1'),
            os.path.join(output_dir.name, 'test_subtask_name', 'file2'),
        ]
        ...
        self.task.options.output_dir = output_dir.name
